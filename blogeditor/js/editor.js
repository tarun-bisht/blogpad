let BlogEditor = function () {
    ...
    const commands = {
        ...
        "format": ["justifyLeft", "justifyCenter", "justifyRight", "foreColor", "backColor", 
        "textSize"],
        "add": ["createLink", "insertHeading","insertCode", "insertImage", "insertHorizontalRule"]
    };
    const toolbar_btn_style="background:#fff;display:block;text-align:center;width:fit-content;height:fit-content;padding:5px;border-radius:1rem 0 1rem 0;margin:5px;border:1px solid #ddd;box-shadow:0 0 3px #aaa;cursor:pointer;";
    const colors = ['000000', 'FF4E00', '061A40', 'FFBE0B', 'CC0000', '00CC00', '0000CC', 'CE2D79', '0A3C78', 'FFFFFF', '320E3B', 'A72608'];
            ...
            while (body.innerText.trim().length > 0) {
                ...
                if (node.nodeName == "#text") {
                    ...
                    if (check_node_for_data_tags(node)) {
                        if (node.tagName == "A") {
                            data.push({ "tag": "A", "link": node.getAttribute("href"), "html": node.innerHTML });
                        }
                        else if (node.tagName == "IMG") {
                            data.push({ "tag": "IMG", "link": node.getAttribute("src"), "alt": node.getAttribute("alt") });
                        }
                        else {
    ...
    function check_node_for_data_tags(node) {
        let data_nodes = ["P", "SPAN", "IMG", "PRE", "CODE", "A", "H1", "H2", "H3", "H4", "H5", "H6","LI"];
        if (data_nodes.indexOf(node.tagName) < 0)
            return false;
        return true;
    }
        ...
        retrieveHTMLFromClipboard(event, function (data) {
            ...
            if (data) {
                ...
                for (let index = 0; index < data.length; index++) {
                    ...
                    if (element["tag"] == "P" || element["tag"] == "LI" || element["tag"] == "H5" || element["tag"] == "H6") {
                    ...
                    else if (element["tag"] == "H1" || element["tag"] == "H2" || element["tag"] == "H3" || element["tag"] == "H4") {
                        inline_container = undefined;
    ...
    function create_paragraph(editable = false, placeholder = "Text Here") {
        ...
        p.addEventListener("paste",function(event)
        {
    ...
    function create_image(imageLink, alt_text = "") {
        ...
        figure.style = "display:flex;flex-flow:column;margin:auto;margin-top:3rem;margin-bottom:3rem;width:fit-content";
    ...
    function create_break() {
        ...
        end.style = "margin:1.5rem 0;position:relative;top:-10px;letter-spacing:5rem;font-size:32px;font-weight:800;text-align:center";
    ...
    function Toolbar() {
        ...
        tool_container.style = "position:sticky;display:flex;flex-wrap:wrap;top:0;left:0;width:100%;justify-content:center;padding:5px";
    ...
    function tool_functionality(cmd) {
        ...
        else if (commands["style"].indexOf(cmd) >= 0 || commands["format"].indexOf(cmd) >= 0) {
        ...
        if(textarea.value)
        {
            ...
            for(let index=0;index < editor_data.length;index++)
            {
                ...
                else if(editor_data[index].tagName=="FIGURE")
                {
                    ...
                    let fig=create_image(image.getAttribute("src"), image.getAttribute("alt"));
    ...
    this.create = function (editorContainer, toolbar = true, heading=true) {
        if (editorContainer.tagName === "TEXTAREA") {
            const editor_container = document.createElement("div");
            editorContainer.parentElement.append(editor_container);
            ...
            editor_container.append(editorContainer);
            setEditorData(editorContainer,editor_div);
            window.onkeyup= function () {
                editorContainer.value = getEditorData(container, true);
	    }
        }
        else {
            throw Error("only textarea can be converted to blog editor")
        }
    }
    this.setEditorToolbar = function (toolbar) {
        for (let index = 0; index < toolbar.childElementCount; index++) {
            ...
            if (cmd.endsWith("Color")) {
                ...
                image_prompt = new PromptBox([{ "placeholder": "Enter Image URL", "value": "" },
                { "placeholder": "Enter alt text", "value": "" }], insertImage, input_style, button_style);
                image_prompt.ok().setAttribute("status", "add");
